            ...
            for line in f:
                ...
                try:
                    ...
                    try:
                        ...
                        print(f"{colored('Invalid age value:', 'yellow')} {colored(age, 'red')}")
                        continue
                    if id and name:
                        for cat in cats_info:
                            if cat["id"] == id:
                                print(f"{colored('Cat with id:', 'yellow')}{colored(id, 'red')} {colored('already exists', 'yellow')}")
                                break
                        else:
                            cats_info.append({"id": id, "name": name, "age": age})
                    else:
                        print(f"{colored('Check data for id:', 'yellow')}{colored(id, 'red')}{colored(', name:', 'yellow')}{colored(name, 'red')}{colored(', age:', 'yellow')}{colored(age, 'red')}")
                except ValueError:
...
def print_directory_structure(directory, prefix=""):
    try:
        ...
        for index, item in enumerate(items):
            ...
            if item.is_dir():
                print(Fore.BLUE + prefix + connector + "📂 " + item.name + Style.RESET_ALL)
...
def add_contact(args, contacts):
    ...
    try:
        ...
        contacts[name] = phone
        file_write(contacts)
        print(f"{colored("Contact added successfully.", 'green')}")
        return contacts
    except ValueError:
...
def change_contact(args, contacts):
    ...
    try:
        ...
        if new_phone is None:
            ...
            file_write(contacts)
            print(f"{colored(f"Contact '{name}' updated successfully.", 'green')}")
            return contacts
    except ValueError:
